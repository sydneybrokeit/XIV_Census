...
generateCharts <- function(pDataSet, location, appendText) {
  ...
  for(i in playerClasses) {;
    ...
    print(ggplot(pDataSubset, aes_string(x=i)) + geom_histogram(binwidth=5, origin=0.5) + geom_histogram(binwidth = 1, color="white", fill="white", origin=0.5) + scale_x_continuous(breaks=levelBreaks, name=paste(pcName, appendText)) + scale_y_continuous(labels=comma));
  ...
  ggplot(grandCompanies, aes(x=grand_company)) + geom_bar(stat="bin") + scale_x_discrete(name=paste("Grand Company", appendText));
  ...
  ggplot(pDataSet, aes(x=race, fill=gender)) + geom_bar(position="dodge") + scale_x_discrete(name=paste("Races", appendText));
